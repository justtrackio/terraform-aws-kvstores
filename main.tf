...
module "kvstore" {
  ...
  ddb_autoscale_read_schedule  = each.value.autoscaler.enabled ? each.value.autoscaler.read_schedule : []
  ddb_autoscale_write_schedule = each.value.autoscaler.enabled ? each.value.autoscaler.write_schedule : []
  ddb_billing_mode             = each.value.table.billing_mode
  ddb_label_order              = var.ddb_label_order

  iam_label_order = var.iam_label_order
  ...
  redis_label_order                        = var.redis_label_order
